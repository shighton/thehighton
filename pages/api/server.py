    ...
    def run():
        ...
        if len(close) > 20:
            ...
            if ((((position >= 0) & able_buy) & should_buy_sma) & (o_sold | (o_bought == False))):
                message = "Position: {position} / Can Buy: {'T' if able_buy else 'F'} / Can Sell: {'T' if able_sell else 'F'} / SMA Buy: {'T' if should_buy_sma else 'F'} / Oversold: {'T' if o_sold else 'F'} / Sell High: {'T' if sell_high else 'F'}"
                ...
                message = "Position: {position} / Can Buy: {'T' if able_buy else 'F'} / Can Sell: {'T' if able_sell else 'F'} / SMA Buy: {'T' if should_buy_sma else 'F'} / Overbought: {'T' if o_bought else 'F'} / Sell High: {'T' if sell_high else 'F'} / Trade Dump: {'T' if no_trades_two_days else 'F'}"
                # if able_sell:
                ...
                cbuy = 'True' if able_buy else 'False'
                csell = 'True' if able_sell else 'False'
                smabuy = 'True' if should_buy_sma else 'False'
                ob = 'True' if o_bought else 'False'
                oso = 'True' if o_sold else 'False'
                sellhi = 'True' if sell_high else 'False'
        else:
            message = "More data needed."
            
        return position, cbuy, csell, smabuy, ob, oso, sellhi

    position, cbuy, csell, smabuy, ob, oso, sellhi = run()
    return jsonify({
        "position" : position,
        "can_buy" : cbuy,
        "can_sell" : csell,
